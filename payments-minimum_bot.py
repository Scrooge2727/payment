...
from config import BOT_TOKEN, PAYMENTS_PROVIDER_TOKEN, TIME_WITCHER_URL
...
# Цена
PRICE = types.LabeledPrice(label='Медальон Геральта из Ривии', amount=5000000)
#Приветствие
...
async def process_buy_command(message: types.Message):
    ...
    await bot.send_invoice(message.chat.id,
                           ...
                           currency='rub',
                           photo_url=TIME_WITCHER_URL,
                           photo_height=512,  # !=0/None, иначе изображение не покажется
                           photo_width=512,
                           photo_size=512,
                           is_flexible=False,  # True если конечная цена зависит от способа доставки
                           prices=[PRICE],
                           start_parameter='witcher-example',
